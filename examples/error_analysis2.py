...
class EmwProcessor2(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train.csv")))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_with_2.csv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "val_with_2.csv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_with_2.csv")), "test")
...
class HyperpartisanData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return input_ids, input_mask, segment_ids, label_ids, ex.guid
...
def main():
    ...
    parser.add_argument("--out_file",
                        ...
                        type=str,
                        required=True,
                        help="The output file for error analysis.")
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if global_step % args.val_each == 0:
                    if args.do_eval:
                        ...
                        for input_ids, input_mask, segment_ids, label_ids, doc_ids in eval_dataloader:
    ...
    if args.do_test:
        ...
        for input_ids, input_mask, segment_ids, label_ids, doc_ids in test_dataloader:
            ...
            for i in range(len(preds)):
                df.iloc[int(doc_ids[i].item()), df.columns.get_loc("prediction")] = int(preds[i])
