...
from pytorch_pretrained_bert.modeling import BertForSequenceClassification_with_facts, BertModel
...
def main():
    ...
    parser.add_argument('--conf_scores',
                        default=False,
                        action='store_true',
                        help="Whether to output confusion matrix and scores as htmls")
    ...
    fact_device = torch.device("cuda:4")
    device_ids = [2,3]
    ...
    # Do we need this in GPU?
    # Fact pool stuff
    fact_model = BertModel.from_pretrained(args.bert_model, PYTORCH_PRETRAINED_BERT_CACHE)
    fact_model.to(fact_device)

    all_fact_inp, all_fact_mask = [], []
    fact_df = pd.read_csv(args.fact_file)
    for fact in fact_df.facts.tolist():
        tokens = tokenizer.tokenize(fact)
        tokens = tokens[0:(args.max_seq_length - 2)]
        tokens = ["[CLS]"] + tokens + ["[SEP]"]
        inp_ids = tokenizer.convert_tokens_to_ids(tokens)
        mask = [1] * len(inp_ids) + [0] * (args.max_seq_length - len(inp_ids))
        inp_ids.extend([0] * (args.max_seq_length - len(inp_ids)))

        all_fact_inp.append(torch.tensor(inp_ids, dtype=torch.long).unsqueeze(0))
        all_fact_mask.append(torch.tensor(mask, dtype=torch.long).unsqueeze(0))

    all_fact_inp = torch.cat(all_fact_inp, dim=0).to(fact_device)
    all_fact_mask = torch.cat(all_fact_mask, dim=0).to(fact_device)
    all_fact_seg = torch.zeros_like(all_fact_inp)

    fact_model.eval()
    _, fact_pool_reps = fact_model(all_fact_inp, all_fact_seg, all_fact_mask) # NxH
    H = fact_pool_reps.shape[1]
    fact_pool_reps = fact_pool_reps.to("cpu")
    fact_pool_reps = torch.cat((fact_pool_reps, torch.zeros(((args.max_seq_length - fact_pool_reps.shape[0]),H))), axis=0) # SxH
    fact_model.to("cpu")
    del fact_model
    fact_pool_reps = fact_pool_reps.data
    # End of fact pool stuff


    # Prepare model
    num_labels = len(label_list)
    model = BertForSequenceClassification_with_facts.from_pretrained(args.bert_model, PYTORCH_PRETRAINED_BERT_CACHE, num_labels=num_labels, fact_reps=fact_pool_reps)
    if args.model_load != "":
    ...
    if args.do_test:
        ...
        if args.error_anal:
            ...
            test_df.to_csv(args.data_dir + "/fact_subset_error_analysis.csv", index=False)
