...
def main():
    ...
    parser.add_argument("--eval_file",
                        default=None,
                        type=str,
                        help="The input evaluation corpus.")
    parser.add_argument("--create_data", # Takes eval file, saves dataset tensors to test_pretrain.pt
                        action='store_true',
                        help="Used for data creation.")
    ...
    if args.create_data:
        dataset = BERTDataset(args.eval_file, tokenizer, seq_len=args.max_seq_length,
                              corpus_lines=None, on_memory=True)
        torch.save(dataset, "test_pretrain.pt")
        raise SystemExit
    ...
    if args.do_eval:
        print("Loading eval Dataset", args.train_file)
        eval_dataset = torch.load(args.eval_file)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if step % 5000 == 0:
                    ...
                    for eval_batch in eval_dataloader:
                        ...
                        input_ids, input_mask, segment_ids, lm_label_ids, is_next = batch
                        cur_loss = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next)
                        if n_gpu > 1:
                    ...
                    if best_loss > cur_loss:                        
                        best_loss = cur_loss
                        logger.info("** ** * Saving fine - tuned model - Loss = " + str(cur_loss) + " ** ** * ")
